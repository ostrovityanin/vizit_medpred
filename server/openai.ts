      ...
      try {
        ...
        const completion = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "Ты эксперт по обработке аудиотранскрипций. НИКОГДА не отвечай метакомментариями и не проси дополнительную информацию! Твоя задача: 1) ВСЕГДА форматировать текст как диалог, даже если кажется, что это монолог одного человека. 2) В монологе просто добавляй префикс 'Говорящий:' перед каждым предложением или логическим блоком. 3) НИКОГДА не изменяй содержание речи, сохраняй абсолютно все слова без изменений. 4) НИКОГДА не добавляй фразы типа 'Извините, но...', 'В данном контексте...', 'Продолжение следует...'. 5) Возвращай ТОЛЬКО отформатированный текст и НИЧЕГО больше. 6) Если не видишь явных признаков диалога, просто пометь весь текст как 'Говорящий:' и верни его."
            },
            {
              role: "user",
              content: `Вот транскрипция аудио: "${transcription}". Отформатируй ее с пометками говорящих. Если это монолог, просто добавь "Говорящий:" перед каждым смысловым блоком. Не меняй сам текст, сохрани все слова, просто добавь метки говорящих.`
            }
          ],
          temperature: 0.1,
          max_tokens: 4000,
