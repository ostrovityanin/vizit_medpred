/**
 * Модуль для работы с OpenAI API (ESM-совместимая версия)
 * 
 ...
 * @returns {Promise<string>} Результат транскрипции
 */
export async function transcribeWithGPT4o(audioFilePath, model = 'gpt-4o', options = {}) {
  const apiKey = process.env.OPENAI_API_KEY;
 ...
 * @returns {Promise<string>} Результат транскрипции
 */
export async function transcribeWithModel(audioFilePath, model, options = {}) {
  switch (model) {
    case 'whisper-1':
